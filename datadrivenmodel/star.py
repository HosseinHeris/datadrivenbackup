...
class Star():
	...
	def __init__(self, predict=False, modeltype='poly'):
		...
		if modeltype=='gb':	
			...
			self.scaler_x_set = joblib.load('scaler_x_set.pkl')
			self.scaler_y_set = joblib.load('scaler_y_set.pkl') 
		...
		if modeltype=='lstm':
			...
			self.markovian_order=1
			self.state_to_brain=self._generate_automated_state_names()



		## this is a temporary section to use approximate reward 
		self.reward_model=joblib.load('./models/gbmodelreward.sav')
...
# initilization of the environment 
	...
	def set_action(self, action):
		...
		if self.modeltype=='lstm':
			...
			newstates=self.scaler_y_set.inverse_transform(newstates)
			...
			for key in action.keys():
				...
				k=k+1
			if self.addnoise==True:
				model_input=np.append(self.state*(1+np.random.uniform(low=-0.1,high=0.1, size=self.state_space_dim)), self.brain_actions)
			else: 
				model_input=np.append(self.state, self.brain_actions)	
		...
		if self.modeltype=='gb':
			...
			#model_input=self.scaler_x_set.transform(model_input)
			self.state=self.scaler_y_set.inverse_transform(self.model.predict(np.array(self.scaler_x_set.transform([model_input]))))
			print('self.state is .. :', self.state)
	...
	def get_terminal(self, state):
		# # the following is an example of state
		# state=np.ravel(self.state)
		...
		notdone = np.isfinite(self.state).all() \
			and self.state[2] >= 0.2 and self.state[2] <= 1.0
		done = not notdone or self.iteration>=self.max_iterations
	...
	def get_reward(self, state, terminal):
		...
		reward=self.reward_model.predict(np.array([self.model_input]))		
		return float(reward)
